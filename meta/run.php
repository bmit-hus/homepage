...
function makeLink($name)
{
    ...
    echo "<a class=\"navobj pl\" href=\"?p=$name\">$linkName</a>";
    add('|');
}
function makeLink_sub($sub, $name)
{
    $linkName = ucfirst($name);
    echo "<a class=\"navobj sl\" href=\"./sub/$sub/$sub.php\">$linkName</a>";
    add('|');
}
...
class Page {
    ...
    function content()
    {
        ...
        echo $parsedown->text(file_get_contents("$root/pages/$this->page/side.md"));
        shut('aside');

        openw('main', 'element main');
        echo $parsedown->text(file_get_contents("$root/pages/$this->page/main.md"));
        shut('main');
    }
}
